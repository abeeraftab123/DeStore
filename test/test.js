const DeStore = artifacts.require('./DeStore.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('DeStore', ([deployer, uploader]) => {
  let destore

  before(async () => {
    destore = await DeStore.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await destore.address
    ...
    it('has a name', async () => {
      const name = await destore.name()
      assert.equal(name, 'DeStore')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await destore.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await destore.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await destore.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await destore.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await destore.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await destore.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await destore.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await destore.files(fileCount)
